...
export default function HomeScreen() {
    ...
    const loadSubscriptions = async (uid: string) => {
        try {
            ...
            const updatedSubs = await Promise.all(subs.map(async (sub) => {
                const parts = sub.nextBilling.split('.');
                if (parts.length !== 3) return sub;
                const nextDate = new Date(Number(parts[2]), Number(parts[1]) - 1, Number(parts[0]));

                if (nextDate <= today) {
                    const newNextBilling = calcNextBilling(sub.startDate, sub.intervalDays);
                    try {
                        await updateDoc(doc(db!, 'subscriptions', sub.id), {nextBilling: newNextBilling});
                    } catch (e) {
                        console.log('Update error:', e);
                    }
                    return {...sub, nextBilling: newNextBilling};
                }
                return sub;
            }));
